...
/**
 * Unit tests for Vision API client in vision-api-client.ts
 */

import { IniadVisionApiClient } from "../src/vision-api-client";
import { secrets } from "../__mocks__/vscode";

// Mock fetch for testing
global.fetch = jest.fn();

describe("IniadVisionApiClient", () => {
  let client: IniadVisionApiClient;

  beforeEach(() => {
    client = new IniadVisionApiClient(secrets);
    jest.clearAllMocks();
  });

  describe("constructor", () => {
    it("should create client with empty API key", () => {
      const newClient = new IniadVisionApiClient(secrets);
  ...
  describe("analyzeImage", () => {
    ...
    it("should call analyze_image tool via INIAD API", async () => {
      ...
      expect(fetchCall[0]).toContain("api.openai.iniad.org");
    });
